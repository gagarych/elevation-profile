...
(function() {
  var ElevationProfile, container, el, initChart, _charts, _i, _j, _len, _len1, _ref;

  ElevationProfile = (function() {
    ElevationProfile.prototype.draw = function() {
      var $area, $bin, $container, $focus, $g, $gmap, $notes, $svg, centerPoint, d, data, dataByX, dist, drawGooglePath, findTextPlacement, gcoord, hideFocus, intersectRect, isValidPlacement, map, marker, measureText, mousemove, notex2, options, opts, place, placement, placements, point, pointsWithPlaces, probingLocations, putNote, setFocus, showFocus, touchmove, x, xAxis, xd, y, yAxis, yByX, yd, _i, _j, _len, _len1, _ref;
  ...
  initChart = function($container) {
    ...
    return d3.json($container.attr('data-src'), function(err, json) {
      ...
      if (err) {
        ...
        return _charts.push(new ElevationProfile($container, json, options));
      }
    });
  };
  ...
  d3.select(window).on('resize', function() {
    var chart, _k, _len2, _results;
    _results = [];
    for (_k = 0, _len2 = _charts.length; _k < _len2; _k++) {
      chart = _charts[_k];
      _results.push(chart.draw());
    }
    return _results;
  });
